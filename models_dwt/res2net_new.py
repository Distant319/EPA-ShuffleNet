...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth = 26, scale = 4, num_classes=8, wavename = 'haar'):
        self.inplanes = 64
        ...
        self.fc = nn.Linear(512 * block.expansion+64, num_classes)
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x1_1, x1_2 = x.chunk(2, dim=1)
        x1_1 = Fourier_filter(x1_1, threshold=1, scale=0.6)
        x1_1 = self.maxpool(x1_1)
        x1_2 = self.maxpool1(x1_2)
