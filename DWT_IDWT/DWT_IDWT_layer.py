...
class DWT_2D_tiny(Module):
    ...
    def __init__(self, wavename):
        ...
        wavelet = pywt.Wavelet(wavename)  #创建一个小波对象，使用给定的波形名称 wavename。小波对象包含有关所选小波的信息，如其滤波器的系数等。
        self.band_low = wavelet.rec_lo
        self.band_high = wavelet.rec_hi   #从小波对象中提取其重构（或逆重构）滤波器的系数。rec_lo 对应于重构滤波器的低通部分，而 rec_hi 对应于重构滤波器的高通部分。
        assert len(self.band_low) == len(self.band_high) #使用 assert 语句来确保低通和高通滤波器的长度相同，因为它们需要在逆小波变换中进行匹配。
        self.band_length = len(self.band_low)  #计算滤波器的长度，并将结果存储在 self.band_length 变量中。
        assert self.band_length % 2 == 0   #使用 assert 语句确保滤波器的长度是偶数。小波滤波器通常具有偶数长度，这是为了方便其在离散小波变换中的使用。
        self.band_length_half = math.floor(self.band_length / 2)  #计算滤波器长度的一半，并将结果存储在 self.band_length_half 变量中。这可能会在后续的代码中用到，以确定滤波器的中心位置。
    ...
    def get_matrix(self):
        ...
        :return: self.matrix_low = \mathcal{L}, self.matrix_high = \mathcal{H}
        """
        L1 = np.max((self.input_height, self.input_width))  #计算输入高度和宽度的最大值，存储在变量 L1 中。
        L = math.floor(L1 / 2)    #计算 L1 的一半并向下取整，结果存储在变量 L 中。
        matrix_h = np.zeros( ( L,      L1 + self.band_length - 2 ) )
        matrix_g = np.zeros( ( L1 - L, L1 + self.band_length - 2 ) )  #创建两个零矩阵，matrix_h 和 matrix_g，用于存储滤波器系数。
        end = None if self.band_length_half == 1 else (-self.band_length_half+1)  #根据 self.band_length_half 的值设置 end 变量，它将在后续的切片操作中使用。
        ...
        #循环生成 matrix_h 矩阵的内容：使用两个嵌套的循环填充 matrix_h，其中外层循环是 L 次，内层循环是 self.band_length 次。
        #通过切片操作，从 matrix_h 中提取部分内容，并将其存储在 matrix_h_0 和 matrix_h_1 中。
        ...
        matrix_h_0 = matrix_h_0[:,(self.band_length_half-1):end]  #进一步切片
        matrix_h_1 = matrix_h_1[:,(self.band_length_half-1):end]
        matrix_h_1 = np.transpose(matrix_h_1)           #进行转置操作。
