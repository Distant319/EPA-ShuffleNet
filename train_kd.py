...
from slefshu_new import shufflenet_v2_x1_0
...
# 定义损失函数，包括知识蒸馏损失和普通交叉熵损失
class DistillationLoss(nn.Module):
    def __init__(self, alpha=0.1, temperature=5):
    ...
    def forward(self, student_outputs, teacher_outputs, labels):
        ...
        # 计算知识蒸馏损失
        distillation_loss = -torch.sum(soft_teacher_outputs * soft_student_outputs) / student_outputs.size(0)
        labels = torch.argmax(labels, dim=1)
        # 计算普通交叉熵损失
        classification_loss = nn.functional.cross_entropy(student_outputs, labels.long())
...
class FeatureDistillationL2Loss(nn.Module):
    def __init__(self):
        super(FeatureDistillationL2Loss, self).__init__()

    def forward(self, student_features, teacher_features):
        # 计算 L2 损失
        loss = nn.MSELoss()(student_features, teacher_features)
...
def train(model, device, train_loader, optimizer, epoch,model_ema):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        x1, x2, x3, x4,output = model(samples)

        optimizer.zero_grad()
        if use_amp:
            with torch.cuda.amp.autocast():
                loss = torch.nan_to_num(criterion_train(output, targets)+0.3*train1(x1,x2)+0.3*train1(x3,x4))
...
if __name__ == '__main__':
    ...
    classes = 8
    resume = None#"checkpoints/ConvNext/model_19_80.415.pth"
    CLIP_GRAD = 5.0
    Best_ACC = 0 #记录最高得分
    use_ema=True
    ...
    train1 = FeatureDistillationL2Loss()
    criterion_val = torch.nn.CrossEntropyLoss()

    model_ft = shufflenet_v2_x1_0(pretrained=False)
    model1 = model_ft.state_dict()
    state_dict = load_state_dict_from_url('C:/Users/MSI-USER/.cache/torch/hub/checkpoints/shufflenetv2_x1-5666bf0f80.pth')
     # 判断预训练模型中网络的模块是否修改后的网络中也存在，并且shape相同，如果相同则取出
    pretrained_dict = {k: v for k, v in state_dict.items() if k in model1 and (v.shape == model1[k].shape)}
    # 更新修改之后的 model_dict
    model1.update(pretrained_dict)
    model_ft.load_state_dict(model1, strict=False)
    print(model_ft)
