...
def val(model, device, test_loader):
    ...
    tsne = TSNE(n_components=2,init='pca',learning_rate=200, metric='cosine', n_jobs=-1)
    ...
    #     plt.plot(temp[:, 0], temp[:, 1], '.', color=css4[lbi])      #plt.plot(temp[:, 0], temp[:, 1], '.', color=css4[lbi],label='Type'+str(lbi+1))
    # plt.tick_params(labelsize=13)
    for lbi, color in zip(range(3), ['r', 'k', 'b']):
        temp = outs_2d[labels == lbi]
        plt.plot(temp[:, 0], temp[:, 1], '.', color=color)
    plt.tick_params(labelsize=13)
    plt.show()
    # plt.title('Dimensionality Reduction Visualization by tSNE')
    # plt.legend()
    # plt.show()
...
if __name__ == '__main__':
    ...
    classes = 3
    resume = "C:/Users/MSI-USER/Downloads/WaveCNet-master/checkpoints/ConvNext/res2+fft_xiaobo+sivit_q/model_23_87.222.pth"
    CLIP_GRAD = 5.0
    Best_ACC = 0 #记录最高得分
    use_ema=True
    ...
    dataset_train = datasets.ImageFolder('D:/ConvNextV2_Demo/data/train', transform=transform)          #data_chang/train
    dataset_test = datasets.ImageFolder("D:/ConvNextV2_Demo/data/val", transform=transform_test)
    ...
    model_ft = res2net50_26w_8s(pretrained=False)
    model_weight_path = "D:/ConvNextV2_Demo/res2net50_26w_8s-2c7c9f12.pth"
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
       ...
       # print(val_list, pred_list)
        fig3 = plt.figure(3)
        C = confusion_matrix(y_true=val_list, y_pred=pred_list, labels=[0, 1, 2])  # 可将'1'等替换成自己的类别，如'cat'。
