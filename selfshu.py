...
class ShuffleNetV2(nn.Module):
    def __init__(
        ...
        for name, repeats, output_channels in zip(
            ...
            # # 将特定层保存为属性
            # if name == 'stage3' or name == 'stage4':
            #     setattr(self, f'{name}_features', nn.Sequential(*seq))  # 保存特定层
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.stage2(x)
        x2 = x
        #x2 = self.stage3_features(x2)
        #x2 = self.stage4_features(x2)
        #print(x2.shape)
        x = self.stage3(x)
        x3_1 = x

        #x3_2 = self.stage4_features(x3_1)
        #print(x3.shape)
        x = self.stage4(x)
        x4 =x
        #print(x4.shape)
    ...
    if pretrained:
        ...
        if model_url is None:
            ...
            for k in ['fc.weight', 'fc.bias']:  # 'fc.weight', 'fc.bias'
                if model1[k].shape != state_dict[k].shape:
                    print('del:',k)
                    del state_dict[k]
